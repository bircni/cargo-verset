...
Write-Host "Please push the changes to the remote repository and publish the crate."

$push_confirmation = Read-Host "Do you want to push the changes? (y/n)"
if ($push_confirmation -eq 'y') {
    git push --follow-tags
    Write-Host "Changes pushed successfully."
}
else {
    Write-Host "Changes not pushed. Exiting."
    exit 0
}

$publish_confirmation = Read-Host "Do you want to publish the crate? (y/n)"
if ($publish_confirmation -eq 'y') {
    cargo publish
    Write-Host "Crate published successfully."
}
else {
    Write-Host "Crate not published. Exiting."
    exit 0
}